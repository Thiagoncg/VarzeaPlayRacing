...
namespace TS.Generics
{
    ...
    public class BezierEditor : Editor
    {
        ...
        SerializedProperty m_diaplayBezierPath;
        SerializedProperty m_drawCurvePointsAndTangents;
        ...
        string[] anchorBlackAndWhite = { "White", "Black (End)" };
        ...
        void OnEnable()
        {
            ...
            m_diaplayBezierPath = serializedObject.FindProperty("diaplayBezierPath");
            m_drawCurvePointsAndTangents = serializedObject.FindProperty("drawCurvePointsAndTangents");
        ...
        void OnSceneGUI()
        {
            ...
            if (m_diaplayBezierPath.boolValue)
                DisplayBezierPath();
            DrawCrossRoadConnection();

            if (m_drawCurvePointsAndTangents.boolValue)
        ...
        void Other()
        {
            ...
            EditorGUILayout.LabelField("");
            EditorGUILayout.LabelField("Curb: ", EditorStyles.boldLabel);
            if (GUILayout.Button("Create", GUILayout.MinWidth(100), GUILayout.Height(40)))
                CreateCurbObject();
        ...
        void SectionConnectRoadToCrossRoad()
        {
            ...
            if (crossRoadIDList.Count == 0)
            {
                ...
                {
                    RoadCross[] roadCross = FindObjectsByType<RoadCross>(FindObjectsInactive.Include, FindObjectsSortMode.None);
        ...
        void DrawHandleLine()
        {
            ...
            for (var i = 0; i < HowManyCurvePoints(); i++)
            {
                if (i % 3 == 1)
                {
                    Vector3 pointPos0 = SListPoint(i - 1);
                    Vector3 pointPos1 = SListPoint(i);
                    Handles.DrawLine(pointPos0, pointPos1, .4f);
                }
                if (i % 3 == 2)
                {
                    Vector3 pointPos0 = SListPoint(i + 1);
        ...
        void CreateCurbObject()
        {
            Bezier bezier = (Bezier)target;

            GameObject curb = (GameObject)PrefabUtility.InstantiatePrefab(bezier.curbPrefab);

            Undo.RegisterCreatedObjectUndo(curb, "curb");


            curb.transform.position = bezier.transform.position;

            // Undo.RegisterFullObjectHierarchyUndo(curb, "Curb");

            SerializedObject serializedObject2 = new UnityEditor.SerializedObject(curb.GetComponent<Curb>());
            SerializedProperty m_roadRefForCurb = serializedObject2.FindProperty("roadRefForCurb");
            serializedObject2.Update();

            m_roadRefForCurb.objectReferenceValue = bezier;
            ...
            /*for (var i = 0; i < 300; i++)
            {
                if(i%5 == 0)
                {
                    var dir = (bezier.distVecList[i].spotPos- bezier.distVecList[i+1].spotPos).normalized;

                    var left = 7.5f * Vector3.Cross(dir, Vector3.up).normalized;
            ...
            Selection.activeGameObject = curb;
