...
namespace TS.Generics
{
    public class TSLightOpti : MonoBehaviour
    {
        ...
        bool IsCurrentTargetDistanceSmallerThanPreviousTargetDistance(int currentID,int lastID = -1)
        {
            ...
            if (isDistanceSmallerThanOtherTarget && isInFrontOfTarget && isDistanceSmallerThanEnableFrontLightDistance ||
                isDistanceSmallerThanOtherTarget && !isInFrontOfTarget && isDistanceSmallerThanEnableBackLightDistance)
                return true;
            else
               return false;
            #endregion
