...
namespace TS.Generics
{
    public class PageIn : MonoBehaviour
    {
        ...
        public void CanvasGroupInteractableSetToTrue(CanvasGroup canvasGrp)
        {
            #region 
            InfoPlayerTS.instance.b_IsPageCustomPartInProcess = true;
            isOperationFinished = false;
        ...
        public void CanvasGroupInteractableSetToFalse(CanvasGroup canvasGrp)
        {
            #region 
            InfoPlayerTS.instance.b_IsPageCustomPartInProcess = true;
            isOperationFinished = false;
        ...
        public void CanvasGroupAlphaSetToOne(CanvasGroup canvasGrp)
        {
            #region 
            InfoPlayerTS.instance.b_IsPageCustomPartInProcess = true;
            isOperationFinished = false;
        ...
        public void CanvasGroupAlphaSetToZero(CanvasGroup canvasGrp)
        {
            #region 
            InfoPlayerTS.instance.b_IsPageCustomPartInProcess = true;
            isOperationFinished = false;
        ...
        public void SetSelectedGameObject(GameObject newSelection)
        {
            #region 
            InfoPlayerTS.instance.b_IsPageCustomPartInProcess = true;
            isOperationFinished = false;
            TS_EventSystem.instance.eventSystem.SetSelectedGameObject(newSelection);
            isOperationFinished = true;
            InfoPlayerTS.instance.b_IsPageCustomPartInProcess = false; 
            #endregion
        }
        ...
        {
            #region 
            StartCoroutine(EnableCameraRoutine(index));
            #endregion
        }

        IEnumerator EnableCameraRoutine(int index)
        {
            InfoPlayerTS.instance.b_IsPageCustomPartInProcess = true;
            isOperationFinished = false;
            var camManager = MenuCameraManager.instance;

            for (var i = 0; i < camManager.grpCamList.Count; i++)
                camManager.grpCamList[i].SetActive(false);
            ...
            for (var i = 0; i < camManager.grpCamList.Count; i++)
            {
                if (i == index)
                {
                    camManager.grpCamList[i].SetActive(true);
                    yield return new WaitUntil(() => camManager.grpCamList[i].activeSelf);
                    break;
                }
            }
            isOperationFinished = true;
