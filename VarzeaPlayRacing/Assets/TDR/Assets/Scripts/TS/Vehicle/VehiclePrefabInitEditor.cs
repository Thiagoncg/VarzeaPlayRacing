...
namespace TS.Generics
{
    ...
    public class VehiclePrefabInitEditor : Editor
    {
        ...
        SerializedProperty m_carModeFive;
        SerializedProperty m_b_AutoInit;
        SerializedProperty m_currentSelectedList;
        ...
        void OnEnable()
        {
            ...
            m_carModeFive = serializedObject.FindProperty("carModeFive");
            m_b_AutoInit = serializedObject.FindProperty("b_AutoInit");
            m_currentSelectedList = serializedObject.FindProperty("currentSelectedList");
        ...
        void SelectModeFiveOrGameMode()
        {
            VehiclePrefabInit myScript = (VehiclePrefabInit)target;
            EditorGUI.BeginChangeCheck();
            if (m_carModeFive.intValue == 0)
            {
                EditorGUILayout.HelpBox("The car is setup to be used in game.", MessageType.Info);
            }
            if (m_carModeFive.intValue == 1)
            {
                EditorGUILayout.HelpBox("The car is ready for Mode 5 (Test the setup of the car). " +
                    "Don't forget to switch to Ready For Game after the modifications.", MessageType.Warning);
            }

            EditorGUILayout.PropertyField(m_carModeFive, new GUIContent(""));
