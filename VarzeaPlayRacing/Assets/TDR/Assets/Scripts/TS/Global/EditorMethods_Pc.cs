// Description : EditorMethods_Pc.cs : Display gameObject, methods and argument in a list.
// Only methods with 0 or 1 argument are allowed. Only public void is allowed.
#if (UNITY_EDITOR)
using UnityEngine;
using UnityEditor;
...
namespace TS.Generics
{
	public class EditorMethods_Pc
	{
		...
		public void DisplayMethodsOnEditor(
			List<EditorMethodsList_Pc.MethodsList> myScript,
			SerializedProperty methodsList,
			GUIStyle style_01,
			bool b_DisplayOnlyTheMethod = false,
			bool b_GUIStyle = true,
			bool b_ShowArgument = true)
		{
			...
			for (var j = 0; j < methodsList.arraySize; j++)
			{
				...
				EditorGUILayout.PropertyField(objRef, new GUIContent(""), GUILayout.MinWidth(50));                              // --> Display gameobject on the Inspector
				...
				if (myScript[j].obj)
				{
					...
					{
						EditorGUI.BeginChangeCheck();
						indexScriptRef.intValue = EditorGUILayout.Popup(indexScriptRef.intValue, options3, GUILayout.MinWidth(50));                 // --> Display all scripts
						...
						if (j != 0)
						{
							if (GUILayout.Button("^", GUILayout.Width(20)))
							{
								methodsList.MoveArrayElement(j, j - 1);
								break;
							}
						}
						else if (methodsList.arraySize > 1)
						{
							if (GUILayout.Button("", GUILayout.Width(20))) { }
						}

						if (j != methodsList.arraySize - 1)
						{
						...
						if (options4.Length > 0)
						{
							...
							indexMethodRef.intValue = EditorGUILayout.Popup(indexMethodRef.intValue, options4, GUILayout.MinWidth(50));             // --> Display all methods



							if (method2s[indexMethodRef.intValue].GetParameters().Length > 0 && b_ShowArgument)
							{                               // Display arguments
		...
		// Display gameObject, methods and argument. Only methods with 0 or 1 argument are allowed. Only public void is allowed.
		public void DisplaySelectedMethodsOnEditor(List<EditorMethodsList_Pc.MethodsList> myScript, SerializedProperty methodsList, GUIStyle style_01, int selectMethods)
		{
			#region
			for (var j = 0; j < methodsList.arraySize; j++)
			{
		...
		// Display gameObject, methods and argument. Only methods with 0 or 1 argument are allowed. Only public void is allowed.
		public void DisplayMethodsOnEditorSaveSection(List<EditorMethodsList_Pc.MethodsList> myScript, SerializedProperty methodsList, GUIStyle style_01)
		{
			#region
			for (var j = 0; j < methodsList.arraySize; j++)
			{
		...
		// Display gameObject, methods and argument. Only methods with 0 or 1 argument are allowed. Only public void is allowed.
		public int find_iSObjectActivatedScript(List<EditorMethodsList_Pc.MethodsList> myScript, SerializedProperty methodsList, GUIStyle style_01)
		{
			#region
			for (var j = 0; j < methodsList.arraySize; j++)
			{
