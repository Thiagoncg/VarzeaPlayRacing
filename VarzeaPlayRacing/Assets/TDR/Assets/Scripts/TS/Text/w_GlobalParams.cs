...
namespace TS.Generics
{
    public class w_GlobalParams : EditorWindow
    {
        ...
        void OnEnable()
        {
            ...
            string objectPath = "Assets/TDR/Assets/Datas/Ref/globalTSDatas.asset";
        ...
        void AccessSavedDataFolder()
        {
            ...
            {
                InfoInputs infoInputs = FindFirstObjectByType<InfoInputs>();
                if (!infoInputs)
                {
            ...
            {
                LoadSavePlayerProgession playerProgression = FindFirstObjectByType<LoadSavePlayerProgession>();
            ...
            {
                if (PlayerPrefs.HasKey("CamP1"))
                    PlayerPrefs.DeleteKey("CamP1");
                if (PlayerPrefs.HasKey("CamP2"))
                    PlayerPrefs.DeleteKey("CamP2");
            }
        ...
        {
            #region 
            ObjectsDependingPlatform[] allObject = FindObjectsByType<ObjectsDependingPlatform>(FindObjectsSortMode.None);
        ...
        {
            #region
            if (helpBox.boolValue) HelpZone(5);

            if (GUILayout.Button("Update Texts"))
            {
                CurrentText[] allObject = FindObjectsByType<CurrentText>(FindObjectsSortMode.None);
                GameObject _TextManager = FindFirstObjectByType<LanguageManager>().gameObject;

                foreach (CurrentText txtChild in allObject)
                {
                    SerializedObject serializedObject3 = new UnityEditor.SerializedObject(txtChild);
                    SerializedProperty _WhichTextData = serializedObject3.FindProperty("_WhichTextData");
                    SerializedProperty _Entry = serializedObject3.FindProperty("_Entry");
                    serializedObject3.Update();


                    SerializedObject serializedObject2 = new UnityEditor.SerializedObject(txtChild);
                    if (txtChild.GetComponent<Text>())
                        serializedObject2 = new UnityEditor.SerializedObject(txtChild.GetComponent<Text>());
                    else
                        serializedObject2 = new UnityEditor.SerializedObject(txtChild.GetComponent<TextMeshProUGUI>());


                    SerializedProperty m_Text = serializedObject2.FindProperty("m_text");
                    if (txtChild.GetComponent<Text>())
                        m_Text = serializedObject2.FindProperty("m_Text");
        ...
        private void HelpZone(int value)
        {
            ...
            switch (value)
            {
                ...
                case 5:
                    EditorGUILayout.HelpBox(
                    "Update all the texts in the current scene (TextMeshPro and Text components). Only activated objects are updated.", MessageType.Info);
                    break;
            }
