...
namespace TS.Generics
{
    public class w_EditLimit : EditorWindow
    {
        ...
        {
            #region 
            LimitCollider objLimit = GameObject.FindFirstObjectByType<LimitCollider>();
            if (objLimit)
            {
                //
                source = objLimit;
        ...
        void OnGUI()
        {
            ...
            if (source)
            {
                editorTab.intValue = GUILayout.SelectionGrid(editorTab.intValue, new string[] { "Path Limit", "Single Collider", "Manual Path" }, 2);
                    ...
                    for (var i = 0; i < source.posList.Count; i++)
                    {
                        if (source.posList[i] == Selection.activeTransform)
                        {
                            ...
                            for (var j = 0; j < 4; j++)
                            {
                                ...
                                {
                                    newSpot.transform.LookAt(source.posList[(i + 1) % howManySpots].position);
                                }
        ...
        public void GenerateMesh(bool bEdit)
        {
            ...
            if (!bEdit)
            {
                ...
                for (var j = 0; j < 4; j++)
                {
                    ...
                    {
                        if (!source.path.TrackIsLooped && (i == 0 || i == source.path.checkpoints.Count - 1 || i == source.path.checkpoints.Count - 2))
                            continue;

                        for (var k = 0; k < source.Subdivision; k++)
                        {
                            if (!source.path.TrackIsLooped && k > 0 && i == source.path.checkpoints.Count - 3)
                                continue;
                            ...
                            {
                                // Case: Last point. Track is not a loop
                                if (!source.path.TrackIsLooped && i == source.path.checkpoints.Count - 2)
                                {
                                    newSpot.transform.position = source.path.checkpoints[i].position;
                                    newSpot.transform.rotation = source.path.checkpoints[i].transform.GetChild(0).rotation;
                                }
                                // other cases
                                else
                                {
                                    ...
                                    {
                                        newSpot.transform.position = source.path.checkpoints[i].position;
                                        newSpot.transform.LookAt(source.path.PositionOnPath(source.path.checkpointsDistanceFromPathStart[i] + .01f, 0));
                                    }
                                    else
                                    {
                                        float dist = source.path.checkpointsDistanceFromPathStart[i + 1] - source.path.checkpointsDistanceFromPathStart[i];
                                        //Debug.Log("dist:" + dist);

                                        float divide = k / (float)source.Subdivision;
                                        //Debug.Log("divide:" + divide);
                                        newSpot.transform.position = source.path.PositionOnPath(source.path.checkpointsDistanceFromPathStart[i] + dist * divide, 0);
                                        newSpot.transform.LookAt(source.path.PositionOnPath(source.path.checkpointsDistanceFromPathStart[i] + dist * (divide + .1f), 0));
                                    }
                                }
                            }
            ...
            if (source.posList.Count > 1)
            {
                ...
                for (var k = 0; k < 4; k++)
                {
                    ...
                    int spots = source.spots + 1;
                    if (!source.path.TrackIsLooped) spots = source.spots;
        ...
        void CreateManualPathCollider()
        {
            #region 
            if (GUILayout.Button("Create Manual Path", GUILayout.Height(40)))
            {
                GameObject newSingleLimit = (GameObject)PrefabUtility.InstantiatePrefab(source.objManualPathLimit);
                Undo.RegisterCreatedObjectUndo(newSingleLimit, newSingleLimit.name);
                Selection.activeGameObject = newSingleLimit;
            }
            #endregion
        }
    }
