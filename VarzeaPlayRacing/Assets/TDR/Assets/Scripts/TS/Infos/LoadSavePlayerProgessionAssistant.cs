    ...
    public class LoadSavePlayerProgessionAssistant : MonoBehaviour
    {
        ...
        public string SaveVehicleInfo()
        {
            ...
            for (var i = 0; i < InfoVehicle.instance.vehicleParametersInGameList.Count; i++)
            {
                ...
                if (i < InfoVehicle.instance.vehicleParametersInGameList.Count - 1) sData += "_";
            }
        ...
        public string SaveChampionshipModeInfo()
        {
            ...
            for (var i = 0; i < GameModeChampionship.instance.listChampionshipPosition.Count; i++)
            {
                ...
                /*if (i < GameModeChampionship.instance.listChampionshipPosition.Count - 1) */
                    sData += "_";
            }

            // Save the last selected event
            sData += GameModeChampionship.instance.currentSelection;
        ...
        public string SaveVehicleCustomization()
        {
            #region
            string sData = "";

            int howManyVehicles = InfoVehicleCustomization.instance.CustomizationVehicleList.Count;
            sData += howManyVehicles;
            sData += "_";

            for (var i = 0; i < InfoVehicleCustomization.instance.CustomizationVehicleList.Count; i++)
            {
                InfoVehicleCustomization.CustomizationParams vehicleParams = InfoVehicleCustomization.instance.CustomizationVehicleList[i];
                int howManyParams = vehicleParams.Vehicle.Count;
                sData += howManyParams;
                sData += "_";
                for (var j = 0; j < vehicleParams.Vehicle.Count; j++)
                {
                    int howManyitems = vehicleParams.Vehicle[j].itemList.Count;
                    sData += howManyitems;
                    sData += "_";

                    int currentSelection =  vehicleParams.Vehicle[j].CurrentSelection;

                   /* for (var k = 1; k < howManyitems; k++)
                        if (vehicleParams.Vehicle[j].itemList[k].IsUnlocked)
                            currentSelection++;
                   */
                    sData += currentSelection;
                    sData += "_";

                    for (var k = 0; k < vehicleParams.Vehicle[j].itemList.Count; k++)
                    {
                        sData += TrueFalse(vehicleParams.Vehicle[j].itemList[k].IsUnlocked);
                        sData += "_";
                    }
                }
            }
        ...
        public bool LoadVehicleCustomization(string sData)
        {
            #region
            InfoVehicleCustomization.instance.Init(sData);
            return true;
            #endregion
        }
