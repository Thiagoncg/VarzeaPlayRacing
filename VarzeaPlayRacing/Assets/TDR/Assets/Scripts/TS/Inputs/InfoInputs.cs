...
namespace TS.Generics
{
    public class InfoInputs : MonoBehaviour
    {
        ...
        public void checkInputs()
        {
            ...
            for (var j = 0; j < ListOfInputsForEachPlayer.Count; j++)
            {
                for (var i = 0; i < ListOfInputsForEachPlayer[j].listOfButtons.Count; i++)
                {
                    ...
                    if (ListOfInputsForEachPlayer[j].listOfButtons[i]._AxisName != "" &&
                        Mathf.Abs(Input.GetAxisRaw(ListOfInputsForEachPlayer[j].listOfButtons[i]._AxisName)) > .2f)
        ...
        public bool checkSpecificInput(int playerID, int buttonID)
        {
            ...
            //-> Check Gamepad
            if (ListOfInputsForEachPlayer[playerID].listOfButtons[buttonID]._AxisName != "" &&
                value <= .2f && 
                Input.GetAxisRaw(ListOfInputsForEachPlayer[playerID].listOfButtons[buttonID]._AxisName) != 0)
            {
