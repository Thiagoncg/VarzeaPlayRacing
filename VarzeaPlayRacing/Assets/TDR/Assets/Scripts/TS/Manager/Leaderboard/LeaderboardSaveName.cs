...
{
	public class LeaderboardSaveName : MonoBehaviour/*, IRewardFeedback<bool> */
	{
		...
		public List<UnityEvent>		GiftList = new List<UnityEvent>();

		public bool					IsGiftProcessDone = true;
		...
		{
            #region 
            StartCoroutine(DisplayGiftRoutine()); 
            #endregion
        }

		IEnumerator DisplayGiftRoutine()
        {
            #region 
            InfoPlayerTS.instance.b_IsPageCustomPartInProcess = true;

            foreach (UnityEvent gift in GiftList)
            {
                IsGiftProcessDone = false;
                gift.Invoke();
                yield return new WaitUntil(() => IsGiftProcessDone);
                Debug.Log("Done");
            }
		...
		public void RewardProcessDone(bool isProcessDone)
        {
            #region 
            IsGiftProcessDone = true; 
            #endregion
        }
	}
}
