...
// Description: CamLookAtConstantDirectionEditor. Custom Editor
#if (UNITY_EDITOR)
using UnityEngine;
using UnityEditor;
using System;
...
{
    [CustomEditor(typeof(CamLookAtConstantDirection))]
    public class CamLookAtConstantDirectionEditor : Editor
    {
        ...
        void OnEnable()
        {
            ...
            m_carSpeed = serializedObject.FindProperty("camSpeed");
            ...
            CamLookAtConstantDirection myScript = (CamLookAtConstantDirection)target;
        ...
        void DisplayGlobalParms()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent("Camera speed:", "The speed used by the camera to follow objLookAtConstantDirectionSpeed"), GUILayout.Width(140));
            m_carSpeed.floatValue = EditorGUILayout.Slider(m_carSpeed.floatValue, 0, 100);
            EditorGUILayout.EndHorizontal();
        ...
        public void ForceUpdateCameraPosition()
        {
            ...
            if (myScript.target)
            {
                ...
                Vector3 offsetLocalPos = myScript.target.right * myScript.lookAtTargetlocalPosition.x + myScript.target.up * myScript.lookAtTargetlocalPosition.y + myScript.target.forward * myScript.lookAtTargetlocalPosition.z;
                Vector3 currentLookAtTargetPos = myScript.target.position + offsetLocalPos;

                myScript.objLookAtConstantDirection.transform.position = currentLookAtTargetPos;

                myScript.transform.position = myScript.objLookAtConstantDirection.transform.position
                    - myScript.objLookAtConstantDirection.transform.forward * myScript.distance
